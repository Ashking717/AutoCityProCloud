...
const ProductSchema = new Schema(
  {
    ...
    yearFrom: {
      ...
      validate: {
        validator: function (this: IProduct, value: number) {
          ...
          if (this.yearTo) return value <= this.yearTo;
          return true;
        },
    ...
    yearTo: {
      ...
      validate: {
        validator: function (this: IProduct, value: number) {
          if (!value) return true;
          if (this.yearFrom) return value >= this.yearFrom;
          return true;
        },
...
// SKU unique per outlet
ProductSchema.index({ outletId: 1, sku: 1 }, { unique: true });

// ✅ PART NUMBER unique per outlet (ignores empty values)
ProductSchema.index(
  { outletId: 1, partNumber: 1 },
  {
    unique: true,
