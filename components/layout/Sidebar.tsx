        ...
        <div className="p-6 overflow-y-auto max-h-[60vh]" style={{ WebkitOverflowScrolling: "touch" }}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <div className="space-y-2">
                {[{ combo: "?", desc: "Show/Hide this dialog" }, { combo: "Esc", desc: "Close dialog/menu" }].map((s) => (
                  <div
                    key={s.combo}
                    className="flex items-center justify-between p-3 rounded-lg transition-colors"
                    style={{ color: th.modalText }}
...
export default function Sidebar({ user, onLogout }: SidebarProps) {
  ...
  const [activeOverlay,     setActiveOverlay]     = useState<'help' | 'mobileMenu' | null>(null);
  ...
  useEffect(() => {
    const handleKeyPress = (e: KeyboardEvent) => {
      ...
      if (e.key === "Escape") {
        if (activeOverlayRef.current) { e.preventDefault(); setActiveOverlay(null); return; }
      }
  ...
  return (
    <>
      ...
      <MobileBottomBar
        primaryMobileNavItems={primaryMobileNavItems}
        pathname={pathname}
        isDark={isDark}
        th={th}
        hasMessagesAccess={hasMessagesAccess}
        ...
        onShowMenu={() => setActiveOverlay('mobileMenu')}
      />
      ...
      {showHelp && (
        <KeyboardShortcutsModal isDark={isDark} th={th} onClose={() => setActiveOverlay(null)} />
      )}
