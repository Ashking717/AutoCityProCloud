...
export default function NewJobPage() {
  ...
  useEffect(() => {
    if (selectedCustomer) {
      setVehicleInfo({
        registrationNumber: selectedCustomer.vehicleRegistrationNumber || "",
  ...
  const handleAddCustomer = async () => {
    ...
    try {
      const code =
        newCustomer.name.toUpperCase().replace(/[^A-Z0-9]/g, "").substring(0, 10) +
  ...
  const calculateTotals = () => {
    ...
    items.forEach((item) => {
      ...
      subtotal += itemTotal;
      const itemDiscount = item.discountType === "percentage" ? (itemTotal * item.discount) / 100 : item.discount;
        ...
        <div className="px-6">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-base text-white/90">Create a new service job or work order</p>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        >
                          <div className="flex justify-between items-start">
                            ...
                            <div className="text-right ml-3">
                              <p className="font-bold text-[#E84545]">QAR {product.sellingPrice}</p>
          ...
          <div className="space-y-6">
              ...
              >
                ...
                {customers.map((customer) => (
                  <option key={customer._id} value={customer._id}>
                    ...
                    {customer.vehicleRegistrationNumber && ` - ${customer.vehicleRegistrationNumber}`}
                  </option>
                ))}
              </select>
              {selectedCustomer && (
                <div className="mb-3 p-3 bg-[#111111] rounded-lg border border-white/5">
                  <p className="text-white font-medium text-sm md:text-base">{selectedCustomer.name}</p>
                  <p className="text-sm text-gray-400">{selectedCustomer.phone}</p>
                  {selectedCustomer.vehicleRegistrationNumber && (
                    <p className="text-xs text-gray-400 mt-1">Vehicle: {selectedCustomer.vehicleRegistrationNumber}</p>
                  )}
                </div>
              )}
              <button
              ...
              <div className="bg-[#0A0A0A] rounded-lg shadow-lg border border-white/5 p-4 md:p-6">
                ...
                <div className="space-y-3">
                  <input
                    ...
                    onChange={(e) => setVehicleInfo({ ...vehicleInfo, registrationNumber: e.target.value.toUpperCase() })}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <select
                      ...
                      onChange={(e) => setVehicleInfo({ ...vehicleInfo, make: e.target.value, model: "" })}
                      className="px-3 py-2 bg-[#111111] border border-white/5 rounded-lg text-white focus:ring-2 focus:ring-[#E84545] text-sm"
                    >
                      <option value="">Make</option>
                      {availableCarMakes.map((make) => <option key={make} value={make}>{make}</option>)}
                    </select>
                    <select
                    ...
                    >
                      ...
                      {isValidCarMake(vehicleInfo.make) &&
                        carMakesModels[vehicleInfo.make].map((model) => <option key={model} value={model}>{model}</option>)}
                    </select>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <select
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      ...
                      className={`px-3 py-2 rounded-lg border text-sm font-medium transition-all flex items-center justify-center gap-2 ${
                        status === "DRAFT" ? "bg-gray-500/20 text-gray-300 border-gray-500/30" : "border-white/5 text-gray-400 hover:bg-white/5"
                      }`}
                    >
                    ...
                    <button
                      ...
                      className={`px-3 py-2 rounded-lg border text-sm font-medium transition-all flex items-center justify-center gap-2 ${
                        status === "PENDING" ? "bg-yellow-500/20 text-yellow-300 border-yellow-500/30" : "border-white/5 text-gray-400 hover:bg-white/5"
                      }`}
                    >
            ...
            <div className="bg-[#0A0A0A] rounded-lg shadow-lg border border-white/5 p-4 md:p-6">
              ...
              <div className="space-y-3">
                <div>
                  <label className="block text-sm font-medium mb-2 text-white">Estimated Start (Date & Time)</label>
                  <input
                    type="datetime-local"
                ...
                <div>
                  <label className="block text-sm font-medium mb-2 text-white">Estimated Completion (Date & Time)</label>
                  <input
                    type="datetime-local"
            ...
            <div className="bg-[#0A0A0A] rounded-lg shadow-lg border border-white/5 p-4 md:p-6">
              <button
                ...
                disabled={loading || isVoiceUploading || !selectedCustomer || !jobTitle.trim()}
        ...
        <div className="md:hidden fixed inset-0 bg-black/80 backdrop-blur-md z-[60]">
          <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-b from-[#0A0A0A] to-[#050505] rounded-t-3xl border-t border-white/10 p-6">
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => setShowMobileMenu(false)} className="p-2 rounded-xl bg-white/5 text-gray-400">
                <X className="h-5 w-5" />
              </button>
            </div>
            <div className="space-y-3">
              <button
                onClick={() => { setShowAddLabor(true); setShowMobileMenu(false); }}
              ...
              <button
                onClick={() => { setShowAddCustomer(true); setShowMobileMenu(false); }}
                className="w-full p-4 bg-[#0A0A0A]/50 border border-white/10 rounded-2xl text-gray-300 font-semibold hover:bg-white/5 transition-all flex items-center justify-between"
              >
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="md:col-span-2">
                  ...
                  <input type="text" value={newCustomer.name} onChange={(e) => setNewCustomer({ ...newCustomer, name: e.target.value })} className="w-full px-3 py-2 bg-[#111111] border border-white/5 rounded-lg text-white" />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1 text-white">Phone *</label>
                  <input type="text" value={newCustomer.phone} onChange={(e) => setNewCustomer({ ...newCustomer, phone: e.target.value })} className="w-full px-3 py-2 bg-[#111111] border border-white/5 rounded-lg text-white" />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1 text-white">Email</label>
                  <input type="email" value={newCustomer.email} onChange={(e) => setNewCustomer({ ...newCustomer, email: e.target.value })} className="w-full px-3 py-2 bg-[#111111] border border-white/5 rounded-lg text-white" />
                </div>
              </div>
              <div className="border-t border-white/5 pt-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <input type="text" value={newCustomer.vehicleRegistrationNumber} onChange={(e) => setNewCustomer({ ...newCustomer, vehicleRegistrationNumber: e.target.value.toUpperCase() })} placeholder="Registration Number" className="w-full px-3 py-2 bg-[#111111] border border-white/5 rounded-lg text-white uppercase md:col-span-2" />
                  <select value={newCustomer.vehicleMake} onChange={(e) => setNewCustomer({ ...newCustomer, vehicleMake: e.target.value, vehicleModel: "" })} className="px-3 py-2 bg-[#111111] border border-white/5 rounded-lg text-white">
                    <option value="">Select Make</option>
                    {availableCarMakes.map((make) => <option key={make} value={make}>{make}</option>)}
                  </select>
                  <select value={newCustomer.vehicleModel} onChange={(e) => setNewCustomer({ ...newCustomer, vehicleModel: e.target.value })} disabled={!newCustomer.vehicleMake} className="px-3 py-2 bg-[#111111] border border-white/5 rounded-lg text-white disabled:opacity-50">
                    <option value="">Select Model</option>
                    {isValidCarMake(newCustomer.vehicleMake) && carMakesModels[newCustomer.vehicleMake].map((model) => <option key={model} value={model}>{model}</option>)}
                  </select>
                </div>
              </div>
              <div className="flex justify-end space-x-3 pt-4">
                <button onClick={() => setShowAddCustomer(false)} className="px-4 py-2 border border-white/5 text-gray-300 rounded-lg">Cancel</button>
                <button onClick={handleAddCustomer} className="px-4 py-2 bg-gradient-to-r from-[#E84545] to-[#cc3c3c] text-white rounded-lg">Add Customer</button>
              </div>
            </div>
          </div>
        </div>
