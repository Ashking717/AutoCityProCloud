...
export default function LedgersPage() {
  ...
  const downloadLedgerCSV = () => {
    ...
    const headers = ["Date", "Voucher Type", "Voucher #", "Narration", "Debit", "Credit", "Balance"];
    ...
    link.setAttribute("download", `ledger_${selectedAccount?.accountName || 'account'}_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  const downloadAccountsCSV = () => {
    ...
    const headers = ["Account Name", "Account Number", "Type", "Group", "Opening Balance", "Current Balance"];
  ...
  // Ledger Detail View
  if (selectedAccount) {
    const accountName = selectedAccount.accountName || selectedAccount.name || 'Account';
    const accountNumber = selectedAccount.accountNumber || selectedAccount.code || '';
    const accountType = (selectedAccount.accountType || selectedAccount.type || '').toLowerCase();
              ...
              <div className="bg-black rounded-[28px] px-6 py-3 shadow-2xl border border-white/10 backdrop-blur-xl pointer-events-auto animate-in slide-in-from-top duration-500">
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-white text-xs font-semibold">{accountName.substring(0, 12)}</span>
                  </div>
                  <div className="h-3 w-px bg-white/20"></div>
                  <div className="flex items-center gap-1.5">
                    <span className="text-white text-xs font-medium">{accountNumber}</span>
                  </div>
                  <div className="h-3 w-px bg-white/20"></div>
            ...
            <div className="px-8">
              <div className="flex justify-between items-center">
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    <h1 className="text-3xl font-bold text-white">{accountName}</h1>
                    <p className="text-white/80 mt-1">{accountNumber} • {accountType}</p>
                  </div>
                </div>
              ...
              <>
                ...
                <div className="md:hidden grid grid-cols-2 gap-3 mb-6">
                  <div className="bg-gradient-to-br from-[#0A0A0A] to-[#050505] border border-white/10 rounded-2xl p-4">
                    ...
                    <p className="text-lg font-bold text-white truncate">QAR {ledgerData.summary.openingBalance.toFixed(2)}</p>
                  </div>
                  <div className="bg-gradient-to-br from-[#0A0A0A] to-[#050505] border border-white/10 rounded-2xl p-4">
                    <div className="flex items-center justify-between mb-2">
                    ...
                    <p className="text-lg font-bold text-red-400 truncate">QAR {ledgerData.summary.totalDebit.toFixed(2)}</p>
                  </div>
                  <div className="bg-gradient-to-br from-[#0A0A0A] to-[#050505] border border-white/10 rounded-2xl p-4">
                    <div className="flex items-center justify-between mb-2">
                      <div className="p-2 bg-green-900/20 rounded-xl">
                    ...
                    <p className="text-lg font-bold text-green-400 truncate">QAR {ledgerData.summary.totalCredit.toFixed(2)}</p>
                  </div>
                  <div className="bg-gradient-to-br from-[#0A0A0A] to-[#050505] border border-white/10 rounded-2xl p-4">
                    <div className="flex items-center justify-between mb-2">
                    ...
                    <p className={`text-lg font-bold ${ledgerData.summary.closingBalance >= 0 ? 'text-green-400' : 'text-red-400'} truncate`}>
                ...
                <div className="hidden md:grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
                  ...
                  <div className="bg-slate-800 border border-slate-700 rounded-xl p-6 hover:border-[#E84545]/30 transition-all">
                    ...
                    <p className={`text-2xl font-bold ${ledgerData.summary.closingBalance >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                      QAR {Math.abs(ledgerData.summary.closingBalance).toFixed(2)}
                    </p>
                  </div>
                </div>
                ...
                <div className="md:hidden">
                  {ledgerData.ledgerEntries.length === 0 ? (
                    <div className="bg-gradient-to-br from-[#0A0A0A] to-[#050505] border border-white/10 rounded-2xl p-8 text-center">
                      ...
                      <p className="text-slate-500 text-sm mt-1">Try adjusting the date range</p>
                    </div>
                  ) : (
                    <div className="space-y-3">
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 mb-2">
                                <span className={`px-2 py-1 text-xs font-semibold rounded-full capitalize ${
                                  entry.voucherType === 'payment' ? 'bg-red-900/30 text-red-400 border border-red-800/50' :
                                  entry.voucherType === 'receipt' ? 'bg-green-900/30 text-green-400 border border-green-800/50' :
                              ...
                              <p className="text-sm font-semibold text-white truncate">{entry.voucherNumber}</p>
                              <p className="text-xs text-slate-400 mt-1 truncate">{entry.narration}</p>
                            </div>
                          </div>

                          <div className="grid grid-cols-3 gap-3 pt-3 border-t border-white/5">
                            <div>
                              <span className="text-[10px] text-slate-500 uppercase block mb-1">Debit</span>
                              <p className="text-sm font-semibold text-red-400">
                                {entry.debit > 0 ? `QAR ${entry.debit.toFixed(2)}` : '-'}
                              </p>
                            </div>
                            <div>
                              <span className="text-[10px] text-slate-500 uppercase block mb-1">Credit</span>
                            ...
                            <div>
                              ...
                              <p className="text-sm font-bold text-white">QAR {entry.balance.toFixed(2)}</p>
                            </div>
                          </div>
                        </div>
                      ))}
                  ...
                  <table className="min-w-full divide-y divide-slate-700">
                    <thead className="bg-slate-900">
                      <tr>
                        ...
                        <th className="px-6 py-3 text-left text-xs font-semibold text-slate-300 uppercase tracking-wider">Type</th>
                        ...
                        <th className="px-6 py-3 text-left text-xs font-semibold text-slate-300 uppercase tracking-wider">Debit</th>
                        <th className="px-6 py-3 text-left text-xs font-semibold text-slate-300 uppercase tracking-wider">Credit</th>
                        <th className="px-6 py-3 text-left text-xs font-semibold text-slate-300 uppercase tracking-wider">Balance</th>
                      </tr>
                    </thead>
                      ...
                      {ledgerData.ledgerEntries.length === 0 ? (
                        <tr>
                          <td colSpan={7} className="px-6 py-12 text-center">
                            ...
                            <p className="text-slate-400 text-lg font-medium">No transactions found for this period</p>
                          </td>
                        </tr>
                      ) : (
                        ledgerData.ledgerEntries.map((entry: any, index: number) => (
                          <tr key={entry._id || index} className="hover:bg-slate-700/50 transition-colors">
                            <td className="px-6 py-4 text-sm text-slate-300">
                              {new Date(entry.date).toLocaleDateString()}
                            </td>
                            <td className="px-6 py-4">
                              <span className={`px-3 py-1.5 inline-flex items-center space-x-2 text-xs font-semibold rounded-full capitalize ${
                                entry.voucherType === 'payment' ? 'bg-red-900/30 text-red-400 border border-red-800/50' :
                                entry.voucherType === 'receipt' ? 'bg-green-900/30 text-green-400 border border-green-800/50' :
                                'bg-blue-900/30 text-blue-400 border border-blue-800/50'
                              }`}>
                                {entry.voucherType === 'payment' ? <TrendingDown className="h-3 w-3" /> :
                                 entry.voucherType === 'receipt' ? <TrendingUp className="h-3 w-3" /> :
                    ...
                    {ledgerData.ledgerEntries.length > 0 && (
                      <tfoot className="bg-slate-900 font-bold">
                        <tr>
                          <td colSpan={4} className="px-6 py-4 text-sm text-right text-slate-300">
                            TOTAL
                          </td>
                          ...
                          <td className={`px-6 py-4 text-sm ${
                            ledgerData.summary.closingBalance >= 0 ? 'text-green-400' : 'text-red-400'
                          }`}>
                            QAR {Math.abs(ledgerData.summary.closingBalance).toFixed(2)}
                          </td>
                        </tr>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-1.5">
                  <span className="text-white text-xs font-medium">
                    {filterType === 'all' ? 'All Types' : accountTypes.find(t => t.value === filterType)?.label}
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="px-8">
            <div className="flex justify-between items-center">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h1 className="text-3xl font-bold text-white">Account Ledgers</h1>
                  <p className="text-white/80 mt-1">View and analyze account transactions</p>
                </div>
              </div>
              <div className="flex space-x-3">
                <button
        ...
        <div className="px-4 md:px-8 pt-[180px] md:pt-6 pb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
            {loading ? (
              ...
              <div className="col-span-full text-center py-12">
                <BookOpen className="h-16 w-16 mx-auto mb-4 text-slate-600" />
                <p className="text-slate-400 text-lg font-medium">No accounts found</p>
              </div>
            ) : (
              filteredAccounts.map((account) => {
                const accountName = account.accountName || account.name || 'Unnamed Account';
                const accountNumber = account.accountNumber || account.code || 'N/A';
                const accountType = (account.accountType || account.type || 'asset').toLowerCase();
                  ...
                  >
                    <div className="flex justify-between items-start mb-4">
                      <div className="flex items-start gap-3 flex-1 min-w-0">
                        <div className={`p-2 rounded-xl ${
                          accountType === 'asset' ? 'bg-green-900/20' :
                          accountType === 'liability' ? 'bg-red-900/20' :
                          accountType === 'equity' ? 'bg-purple-900/20' :
                          accountType === 'revenue' ? 'bg-blue-900/20' :
                          'bg-orange-900/20'
                        }`}>
                          {getAccountTypeIcon(accountType)}
                        </div>
                        <div className="flex-1 min-w-0">
                          <h3 className="text-base md:text-lg font-bold text-white truncate">{accountName}</h3>
                          <p className="text-xs md:text-sm text-slate-500 mt-1 truncate">{accountNumber}</p>
                        </div>
                      </div>
                    ...
                    <div className="space-y-2 mb-4 md:mb-6">
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-medium text-slate-300">{accountGroup}</span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className="text-slate-500">Opening Balance:</span>
                        <span className="font-semibold text-slate-300">QAR {openingBalance.toFixed(2)}</span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className="text-slate-500">Current Balance:</span>
                        <span className={`font-bold ${currentBalance >= 0 ? 'text-green-400' : 'text-red-400'}`}>
