...
export default function PaymentVoucherPage() {
  ...
  const removeEntry = (index: number) => {
    if (entries.length <= 2) {
      toast.error('At least 2 entries required for payment voucher');
      return;
    }
  ...
  const handleSubmit = async (status: 'draft' | 'posted') => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        toast.success(`Payment voucher ${status === 'draft' ? 'saved' : 'posted'} successfully!`);
        router.push(`/autocityPro/vouchers/${data.voucher._id}`);
      } else {
        const error = await res.json();
        toast.error(error.error || 'Failed to create payment voucher');
      }
    } catch (error) {
    ...
    <MainLayout user={user} onLogout={handleLogout}>
          ...
          <div className="flex justify-between items-center">
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-white">Payment Voucher</h1>
                  <p className="text-purple-100">Record payments made</p>
                </div>
              </div>
            </div>
      ...
      <div className="p-8 bg-slate-800 min-h-screen">
        <div className="max-w-6xl mx-auto">
            ...
            {showHelp && (
              <div className="mt-2 bg-slate-900/50 border border-slate-700 rounded-lg p-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <h3 className="text-lg font-semibold text-slate-100 mb-3">Payment Voucher Basics</h3>
                    <p className="text-slate-300 mb-4">
                      A <strong>Payment Voucher</strong> documents money paid out by the business. 
                      It's used to record expenses, supplier payments, salaries, bills, and other outgoing payments.
                    </p>
                    ...
                    <div className="space-y-3">
                      <div className="flex items-start space-x-3">
                        ...
                        <div>
                          <h4 className="text-sm font-medium text-slate-100">Debit Side</h4>
                          <p className="text-sm text-slate-400">Expense or asset account receiving the benefit</p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-start space-x-3">
                        ...
                        <div>
                          <h4 className="text-sm font-medium text-slate-100">Credit Side</h4>
                          <p className="text-sm text-slate-400">Cash or Bank account that paid the money</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="mt-4 p-3 bg-slate-800/30 rounded-lg border border-slate-700">
                      <p className="text-sm text-slate-300 italic">
                        <strong>Golden Rule:</strong> "Credit what goes out" - When paying money, credit Cash/Bank account
                      </p>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="mt-4 p-3 bg-red-900/20 rounded-lg border border-red-800/50">
                      <p className="text-sm text-red-300">
                        <strong>Tip:</strong> Start with Expense/Asset account on Debit side when making payments
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-4 pt-4 border-t border-slate-700">
                  ...
                  <ol className="list-decimal list-inside space-y-2 text-sm text-slate-300">
                    <li>Select payment date and add description</li>
                    <li>Add at least 2 entries (one debit, one credit)</li>
                    <li>Debit the Expense/Asset account receiving benefit</li>
                    <li>Credit Cash/Bank account that made the payment</li>
                    <li>Ensure total debits = total credits</li>
          ...
          {/* Information Banner */}
          <div className="bg-red-900/20 border-l-4 border-red-500 p-4 mb-6 rounded-lg border border-red-800/30">
            <div className="flex items-start">
              <AlertCircle className="h-5 w-5 text-red-400 mr-3 mt-0.5 flex-shrink-0" />
              <div>
                <p className="text-sm text-red-300">
                  <strong>Payment Voucher</strong> is used to record payments made to suppliers, vendors, 
                  or for expenses. Typically includes credit to Cash/Bank account.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-900 rounded-lg border border-slate-700 p-6 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label htmlFor="payment-date" className="block text-sm font-medium text-slate-300 mb-1">
              ...
              <div>
                <label htmlFor="payment-ref" className="block text-sm font-medium text-slate-300 mb-1">Reference Number</label>
                <input
                  id="payment-ref"
                  type="text"
                  value={formData.referenceNumber}
                  onChange={(e) => setFormData({ ...formData, referenceNumber: e.target.value })}
                  placeholder="Optional (e.g., Invoice #, PO #)"
                  className="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent text-slate-100"
                />
              </div>
              
              <div>
                <label htmlFor="payment-desc" className="block text-sm font-medium text-slate-300 mb-1">
                  Payment Description <span className="text-red-400">*</span>
                </label>
                <input
                  id="payment-desc"
                  type="text"
                  ...
                  placeholder="What was this payment for? (e.g., Payment to supplier)"
                  className="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent text-slate-100"
                />
              </div>
            </div>
          ...
          <div className="bg-slate-900 rounded-lg border border-slate-700 overflow-hidden mb-6">
            <div className="px-6 py-4 border-b border-slate-700 flex justify-between items-center">
              <div>
                <h2 className="text-lg font-semibold text-slate-100">Accounting Entries</h2>
                <p className="text-sm text-slate-400 mt-1">Debit: What you get | Credit: What you give</p>
              </div>
              <button
                onClick={addEntry}
                    ...
                    <tr key={entry.id} className="hover:bg-slate-800/50 transition-colors">
                      ...
                      <td className="px-4 py-4">
                        <input
                          ...
                          placeholder="Entry description (e.g., Office supplies purchase)"
                          className="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent text-slate-100"
                        />
                      </td>
                      <td className="px-4 py-4">
          ...
          <div className="bg-slate-900 rounded-lg border border-slate-700 p-6 mb-6">
            <h3 className="text-lg font-semibold text-slate-100 mb-4">Common Payment Types</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <div className="p-3 bg-slate-800/30 rounded-lg border border-slate-700">
                <h4 className="text-sm font-medium text-slate-100 mb-1">Supplier Payments</h4>
                <p className="text-xs text-slate-400">Debit: Purchases/Inventory, Credit: Cash/Bank</p>
              </div>
              <div className="p-3 bg-slate-800/30 rounded-lg border border-slate-700">
                <h4 className="text-sm font-medium text-slate-100 mb-1">Salary Payments</h4>
                <p className="text-xs text-slate-400">Debit: Salary Expense, Credit: Bank Account</p>
              </div>
              <div className="p-3 bg-slate-800/30 rounded-lg border border-slate-700">
                <h4 className="text-sm font-medium text-slate-100 mb-1">Expense Payments</h4>
                <p className="text-xs text-slate-400">Debit: Expense Account, Credit: Cash Account</p>
              </div>
              <div className="p-3 bg-slate-800/30 rounded-lg border border-slate-700">
                <h4 className="text-sm font-medium text-slate-100 mb-1">Rent Payments</h4>
                <p className="text-xs text-slate-400">Debit: Rent Expense, Credit: Bank Account</p>
              </div>
              <div className="p-3 bg-slate-800/30 rounded-lg border border-slate-700">
                <h4 className="text-sm font-medium text-slate-100 mb-1">Loan Repayments</h4>
                <p className="text-xs text-slate-400">Debit: Loan Payable, Credit: Cash/Bank</p>
              </div>
              <div className="p-3 bg-slate-800/30 rounded-lg border border-slate-700">
                <h4 className="text-sm font-medium text-slate-100 mb-1">Utility Bills</h4>
                <p className="text-xs text-slate-400">Debit: Utility Expense, Credit: Bank Account</p>
              </div>
            </div>
          </div>
