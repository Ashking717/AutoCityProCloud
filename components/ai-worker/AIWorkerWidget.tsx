...
export function AIWorkerWidget({
  ...
  // ── Voice input (Realtime → push-to-talk fallback) ─────────────────────────
  const voice = useVoiceInput({
    // Push-to-talk path: transcript drops into the chat input
    onTranscript: (text) => {
      setInput(text);
      // Auto-send after a short delay so user can see what was heard
      setTimeout(() => {
        setInput('');
        sendMessage(text);
      }, 800);
    ...
    <>
      ...
      {isOpen && (
        ...
        >
          ...
          {/* Scan invoice banner (mid-conversation) */}
          {messages.length > 1 && !isLoading && (
          ...
          {/* Live voice banner — realtime mode only */}
          {voice.isLive && (
            <div
            ...
            >
              ...
              <div className="flex-1">
                <span className="text-xs" style={{ color: th.voiceBannerText }}>
          ...
          {voice.isRecording && (
            ...
            >
              ...
              />
              <span className="text-xs flex-1" style={{ color: th.voiceBannerText }}>
          ...
          {/* Input bar */}
          <div className="px-3 py-3 flex gap-2 flex-shrink-0" style={{ borderTop: `1px solid ${th.inputBarBorder}` }}>
            <input
              ref={inputRef}
              type="text"
              autoFocus
              ...
              style={{ background: th.inputBg, border: `1px solid ${th.inputBorder}`, color: th.inputText }}
