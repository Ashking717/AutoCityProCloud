...
export default function ReceiptVoucherPage() {
  ...
  const removeEntry = (index: number) => {
    if (entries.length <= 2) {
      toast.error('At least 2 entries required for receipt voucher');
      return;
    }
  ...
  const handleSubmit = async (status: 'draft' | 'posted') => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        toast.success(`Receipt voucher ${status === 'draft' ? 'saved' : 'posted'} successfully!`);
        router.push(`/autocityPro/vouchers/${data.voucher._id}`);
      } else {
        const error = await res.json();
        toast.error(error.error || 'Failed to create receipt voucher');
      }
    } catch (error) {
  ...
  const exampleTransactions = [
    ...
    {
      description: 'Bank deposit from client',
      debit: 'Bank Account',
    ...
    <MainLayout user={user} onLogout={handleLogout}>
          ...
          <div className="flex justify-between items-center">
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-white">Receipt Voucher</h1>
                  <p className="text-purple-100">Record receipts received</p>
                </div>
              </div>
            </div>
      ...
      <div className="p-8 bg-slate-800 min-h-screen">
        <div className="max-w-6xl mx-auto">
            ...
            {showHelp && (
              <div className="mt-2 bg-slate-900/50 border border-slate-700 rounded-lg p-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <h3 className="text-lg font-semibold text-slate-100 mb-3">Receipt Voucher Basics</h3>
                    <p className="text-slate-300 mb-4">
                      A <strong>Receipt Voucher</strong> documents money received by the business. 
                      It's used to record income, customer payments, loans received, and other receipts.
                    </p>
                    ...
                    <div className="space-y-3">
                      <div className="flex items-start space-x-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-slate-400">Cash or Bank account receives the money</p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-start space-x-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-slate-400">Income source (Sales, Loans, etc.)</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="mt-4 p-3 bg-slate-800/30 rounded-lg border border-slate-700">
                      <p className="text-sm text-slate-300 italic">
                        <strong>Remember:</strong> Total Debit must equal Total Credit (Double-entry system)
                      </p>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="mt-4 p-3 bg-green-900/20 rounded-lg border border-green-800/50">
                      <p className="text-sm text-green-300">
                        <strong>Tip:</strong> Start with Cash/Bank account on Debit side when receiving money
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-4 pt-4 border-t border-slate-700">
                  ...
                  <ol className="list-decimal list-inside space-y-2 text-sm text-slate-300">
                    <li>Select date and add narration</li>
                    <li>Add at least 2 entries (one debit, one credit)</li>
                    <li>Debit Cash/Bank account (money received)</li>
                    <li>Credit appropriate income/receivable account</li>
          ...
          {/* Information Banner */}
          <div className="bg-green-900/20 border-l-4 border-green-500 p-4 mb-6 rounded-lg border border-green-800/30">
            <div className="flex items-start">
              <AlertCircle className="h-5 w-5 text-green-400 mr-3 mt-0.5 flex-shrink-0" />
              <div>
                <p className="text-sm text-green-300">
                  <strong>Receipt Voucher</strong> is used to record receipts from customers, debtors, 
                  or other income. Typically includes debit to Cash/Bank account.
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Receipt description (e.g., Payment from customer)"
                  className="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent text-slate-100"
                />
              </div>
            </div>
          ...
          <div className="bg-slate-900 rounded-lg border border-slate-700 overflow-hidden mb-6">
            <div className="px-6 py-4 border-b border-slate-700 flex justify-between items-center">
              ...
              <button
                onClick={addEntry}
                className="flex items-center space-x-2 px-4 py-2 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-lg hover:opacity-90 transition-opacity"
              >
          ...
          <div className="flex justify-end space-x-3">
            ...
            <button
              ...
              className="flex items-center space-x-2 px-6 py-3 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-lg hover:opacity-90 disabled:opacity-50 transition-opacity"
            >
              {loading ? (
                <>
